...
@Component({
  ...
  styleUrls: ["./pojo-display.component.css"]
})
export class PojoDisplayComponent implements OnInit {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (
      changes.details &&
      changes.details.currentValue !== changes.details.previousValue
    ) {
      this.field = this.details.id ? this.details.id : this.details.type;
      this.hasFields =
        this.details.properties && this.details.properties.length > 0;
    }
  }

  ngOnInit() {
    // thisdetails = this.treeData;
  ...
  checkSelections(parent) {
    ...
    for (let child of parent.properties) {
      if (child.properties) this.checkSelections(child);
      if (child.selected) selectParent = true;
    }
  ...
  setChildren(parent, selected) {
    for (let child of parent.properties) {
      ...
      if (child.properties) this.setChildren(child, selected);
    }
  }
}
